...
if (typeof window.BOOT_SEQUENCES === 'undefined') {
	window.BOOT_SEQUENCES = window.bootSequences || {
	professional: [
...
window.BOOT_CONFIG = {
	professional: {
		messages: window.BOOT_SEQUENCES.professional,
		...
		mobileMessages: window.BOOT_SEQUENCES.professionalMobile,
		mobileFontSize: '1.2rem', // Smaller font for mobile
	},

	cyberpunk: {
		messages: window.BOOT_SEQUENCES.cyberpunk,
		messageDelay: 800, // 0.8 seconds between messages
		finalPause: 600, // Pause before fade-out
		fadeOutDuration: 800, // Fade-out animation duration
		mobileMessages: window.BOOT_SEQUENCES.cyberpunkMobile,
		mobileFontSize: '1.2rem', // Smaller font for mobile
	},
};
}

// Use the global BOOT_CONFIG without redeclaring
...
function createBootSequence(heroStyle = "professional", terminalContent) {
	...
	const config = window.BOOT_CONFIG[heroStyle] || window.BOOT_CONFIG.professional;
  ...
  `;
	...
	// Only set content-specific properties if needed
	if (fontSize !== '1.5rem') {
		bootElement.style.fontSize = fontSize;
	}

	// Append the boot element to the glitch element (title)
	// It will be absolutely positioned below the title
	debugBoot("Appending boot element to glitch element");
...
// Make createBootSequence available globally
window.createBootSequence = createBootSequence;
